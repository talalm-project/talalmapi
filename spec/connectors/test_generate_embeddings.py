...
def test_generate_embeddings_openai_connector_returns_records(client, app, db_session, monkeypatch):
    user = UserFactory(role="user")
    connector = ConnectorFactory(
        user=user,
    ...
    monkeypatch.setattr(
        "app.operations.embeddings.generate_openai_embeddings._openai_client_class",
        lambda: FakeOpenAI,
    )
    ...
    response = client.post(
        f"/connectors/{connector.id}/generate_embeddings",
        headers=_headers(app, user),
        files={"file": ("notes.txt", b"hello openai", "text/plain")},
    )

    assert response.status_code == 200
    payload = response.json()
    assert payload["records"][0]["text"] == "hello openai"
    assert payload["records"][0]["embedding"] == [0.1, 0.2, 0.3]
    assert payload["records"][0]["metadata"]["source_name"] == "notes.txt"
    assert payload["records"][0]["metadata"]["model"] == "text-embedding-3-small"
...
def test_generate_embeddings_openai_connector_uses_metadata_model_and_dimensions(client, app, db_session, monkeypatch):
    user = UserFactory(role="user")
    connector = ConnectorFactory(
        user=user,
        connection_type="openai",
        local_file_path=None,
        embedding_name="column-embedding",
        api_key="sk-secret",
        data={
            "metadata": {
                "embeddings": {
    ...
    assert response.json()["records"][0]["metadata"]["model"] == "text-embedding-3-large"
    assert response.json()["records"][0]["metadata"]["embedding_size"] == 1024
...
def test_generate_embeddings_openai_requires_embedding_name(client, app, db_session):
    user = UserFactory(role="user")
    connector = ConnectorFactory(
        user=user,
        connection_type="openai",
    ...
    assert response.status_code == 422
    assert response.json()["embedding_name"] == ["required"]
